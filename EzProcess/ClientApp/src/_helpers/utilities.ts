...
export class Utilities {
  ...
  public static cookies =
    {
      getItem: (sKey: any) => {
        return decodeURIComponent(document.cookie.replace(new RegExp('(?:(?:^|.*;)\\s*' + encodeURIComponent(sKey).replace(/[\-\.\+\*]/g, '\\$&') + '\\s*\\=\\s*([^;]*).*$)|^.*$'), '$1')) || null as any;
      },
  ...
  public static splitInTwo(text: string, separator: string): { firstPart: string, secondPart: string } {
    ...
    if (separatorIndex == -1) {
      return { firstPart: text, secondPart: null as any };
    }
  ...
  public static isNotNullOrUndefined(obj: any) {
    if (obj !== null && obj !== undefined && obj !== 'undefined') {
      return true;
    }
    return false;
